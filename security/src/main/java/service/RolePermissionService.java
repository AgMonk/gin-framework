...
import bo.RelationUserRoleBo;
import bo.SystemUserBo;
import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import dto.form.RegForm;
...
public class RolePermissionService implements AuthorityProvider {
    ...
    public List<RelationRolePermission> addRolePermissionWithPath(
        ...
        if (!CollectionUtils.isEmpty(path)) {
            ...
            for (String pattern : path) {
                idSet.addAll(permissions.stream().filter(perm -> antPathMatcher.match(pattern, perm.getPath())).map(BaseFields::getId).toList());
            }
        }
        //按分组名添加
        if (!CollectionUtils.isEmpty(groupName)) {
            idSet.addAll(permissions.stream().filter(perm -> groupName.contains(perm.getGroupName())).map(BaseFields::getId).toList());
        }
    ...
    public void forbiddenConfigAdminUser(long userId) {
        ...
        if (authorities.contains(roleAdmin)) {
            throw BusinessException.of(HttpStatus.FORBIDDEN, Security.NOT_CONFIG_ADMIN);
        }
    }
